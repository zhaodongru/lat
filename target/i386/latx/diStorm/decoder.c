...
{
    /*
     * This table is to map from the current decoding mode to an effective
     * address size: Decode16 -> Decode32 Decode32 -> Decode16 Decode64 ->
     * Decode32
     */
    static _DecodeType AddrSizeTable[] = {Decode32Bits, Decode16Bits,
                                          Decode32Bits};
                                                ...
                                                _iflags instFlags)
{
    /*
     * This table is to map from the current decoding mode to an effective
     * operand size: Decode16 -> Decode32 Decode32 -> Decode16 Decode64 ->
     * Decode16 Not that in 64bits it's a bit more complicated, because of REX
     * and promoted instructions.
     */
    static _DecodeType OpSizeTable[] = {Decode32Bits, Decode16Bits,
                                        Decode16Bits};
    ...
    if (dt == Decode64Bits) {
        ...
        if (((instFlags & (INST_64BITS | INST_PRE_REX)) == INST_64BITS) ||
            /* Other instructions in 64 bits can be promoted only with a REX
               prefix. */
...
static _DecodeResult decode_inst(_CodeInfo* ci, _PrefixState* ps, _DInst* di)
{
    ...
    /*
     * Indicates whether it is right to LOCK the instruction by decoding its
     * first operand. Only then you know if it's ok to output the LOCK prefix's
     * text... Used for first operand only.
     */
    int lockable = FALSE;

    /* Calcualte (and cache) effective-operand-size and effective-address-size
     * only once. */
     ...
     * because REX has precedence. However, only if REX.W is set ! We had to
     * wait with this test, since the operand size may be a mandatory prefix,
     * and we know it only after prefetching.
     */
    if ((ps->prefixExtType == PET_REX) &&
        (ps->decodedPrefixes & INST_PRE_OP_SIZE) && (!ps->isOpSizeMandatory) &&
        (vrex & PREFIX_EX_W)) {
     ...
     * If this is the case, drop what we've got and restart all over after
     DB'ing that byte.
     ...
     * BITS INSTRUCTIONS ! ! !*/
    /* if (ii && (dt == Decode16Bits) && (instFlags & INST_32BITS) &&
     * (~instFlags & INST_16BITS)) ii = NULL; */
    ...
    if (instFlags & INST_MODRM_REQUIRED) {
        /* If the ModRM byte is not part of the opcode, skip the last byte code,
         * so code points now to ModRM. */
        ...
        /* Some instructions enforce that reg=000, so validate that.
         * (Specifically EXTRQ). */
        if ((instFlags & INST_FORCE_REG0) && (((modrm >> 3) & 7) != 0))
            goto _Undecodable;
        /* Some instructions enforce that mod=11, so validate that. */
        if ((instFlags & INST_MODRR) && (modrm < INST_DIVIDED_MODRM))
            goto _Undecodable;
    }

    ci->code++; /* Skip the last byte we just read (either last opcode's byte
                   code or a ModRM). */

    /* Cache the effective operand-size and address-size. */
    effOpSz = decode_get_effective_op_size(ci->dt, ps->decodedPrefixes, vrex,
                                           instFlags);
     ...
     * For example, if there is not first operand, no reason to try to extract
     * second operand... I decided that a for-break is better for readability in
     * this specific case than goto. Note: do-while with a constant 0 makes the
     * compiler warning about it.
     */
    for (;;) {
        if (ii->d != OT_NONE) {
            if (!operands_extract(ci, di, ii, (_OpType)ii->d, ONT_1, modrm, ps,
                                  effOpSz, effAdrSz, &lockable))
                goto _Undecodable;
        } else
            break;

        if (ii->s != OT_NONE) {
            if (!operands_extract(ci, di, ii, (_OpType)ii->s, ONT_2, modrm, ps,
                                  effOpSz, effAdrSz, NULL))
                goto _Undecodable;
        } else
            break;
        ...
        if (instFlags & INST_USE_OP3) {
            if (!operands_extract(ci, di, ii, (_OpType)((_InstInfoEx*)ii)->op3,
                                  ONT_3, modrm, ps, effOpSz, effAdrSz, NULL))
                goto _Undecodable;
        } else
            break;
        ...
        if (instFlags & INST_USE_OP4) {
            if (!operands_extract(ci, di, ii, (_OpType)((_InstInfoEx*)ii)->op4,
                                  ONT_4, modrm, ps, effOpSz, effAdrSz, NULL))
                goto _Undecodable;
        }
        break;
    ...
    /*
     * There's a limit of 15 bytes on instruction length. The only way to
     * violate this limit is by putting redundant prefixes before an
     * instruction. start points to first prefix if any, otherwise it points to
     ...
     * necessarily mean we will choose mnemonic2, alas, it means that if there
     * is a mnemonic2, it will be used.
     */
    ...
    if ((lockable == TRUE) && (instFlags & INST_PRE_LOCK)) {
        ...
        di->flags |= FLAG_LOCK;
    } else if ((instFlags & INST_PRE_REPNZ) &&
    ...
    if ((instFlags & (INST_PRE_ADDR_SIZE | INST_USE_EXMNEMONIC)) ==
        (INST_PRE_ADDR_SIZE | INST_USE_EXMNEMONIC)) {
    ...
    else if ((instFlags & (INST_PRE_ADDR_SIZE | INST_NATIVE)) ==
             (INST_PRE_ADDR_SIZE | INST_NATIVE)) {
        di->opcode = ii->opcodeId;

        /* If LOOPxx gets here from 64bits, it must be Decode32Bits because
         * Address Size perfix is set. */
        ps->usedPrefixes |= INST_PRE_ADDR_SIZE;
    }
    /*
     * Note:
     * If the instruction is prefixed by operand size we will format it in the
     * non-default decoding mode! So there might be a situation that an
     * instruction of 32 bit gets formatted in 16 bits decoding mode. Both ways
     * should end up with a correct and expected formatting of the text.
     */
        ...
        /*
         * If it's a special instruction which has two mnemonics, then use the
         * 16 bits one + update usedPrefixes. Note: use 16 bits mnemonic if that
         * instruction supports 32 bit or 64 bit explicitly.
         */
        if ((instFlags & INST_USE_EXMNEMONIC) &&
            ((instFlags & (INST_32BITS | INST_64BITS)) == 0))
        ...
        if (instFlags & INST_USE_EXMNEMONIC) {
            ...
            /* Or is it a special CMP instruction which needs a pseudo opcode
               suffix ? */
            else if (instFlags & INST_PSEUDO_OPCODE) {
                /*
                 * The opcodeId is the base id of the pseudo compare mnemonics,
                 * thus we can access the next ones from 0 to 7 safely, because
                 * they are already allocated.
                 */
#ifdef _DISTORM_NO_DEBUG
                di->opcode = ii->opcodeId + cmpType;
#else
                int i;
                unsigned int stringOffset = 0;
                /*
                 * The _MNEMONICS string contains pascal-style strings with the
                 * length of each mnemonic specified. We will sum each mnemonic
                 * length to arrive at the final mnemonic offset.
                 */
                for (i = 0; i < cmpType; i++) {
                    /* +1 is for the null terminator, +1 is for the length byte
                     * itself */
            ...
            di->opcode = ii->opcodeId;
    } else { /* Decode64Bits, note that some instructions might be decoded in
                Decode32Bits above. */
        ...
        if (instFlags & (INST_USE_EXMNEMONIC | INST_USE_EXMNEMONIC2)) {
            ...
            if ((instFlags & INST_USE_EXMNEMONIC2) && (vrex & PREFIX_EX_W)) {
                ...
                di->opcode =
                    ((_InstInfoEx*)ii)->opcodeId2; /* Use second mnemonic. */
        } else
            di->opcode = ii->opcodeId;
    }
...
_Undecodable: /* If the instruction couldn't be decoded for some reason, drop
                 the first byte. */
    memset(di, 0, sizeof(_DInst));
    di->base = R_NONE;
    ...
    /* Special case for WAIT instruction: If it's dropped, you have to return a
     * valid instruction! */
    ...
    /* Mark that we didn't manage to decode the instruction well, caller will
     * drop it. */
...
_DecodeResult decode_instruction(uint8_t* ins_addr, _DInst* pdi, _DecodeType dt)
{
    const uint8_t* code = (uint8_t*)ins_addr;
    int codeLen = 32;

    // variable features will be used to determine the addrMask, but it is not
    // used yet.
    /* unsigned int features = DF_NONE;
    if (dt == Decode16Bits) features = DF_MAXIMUM_ADDR16;
    else if (dt == Decode32Bits) features = DF_MAXIMUM_ADDR32;*/
    ...
    /* startInstAddr holds the displayed offset of current instruction. */
    _OffsetType startInstAddr = (_OffsetType)code;
    ...
    if (prefixes_is_valid(*code, dt)) {
        prefixes_decode(code, codeLen, &ps, dt);
        /* Count prefixes, start points to first prefix. */
        prefixSize = (unsigned int)(ps.last - ps.start);
        /*
         * It might be that we will just notice that we ran out of bytes, or
         * only prefixes so we will have to drop everything and halt. Also take
         * into consideration of flow control instruction filter.
         */
        codeLen -= prefixSize;
        if ((codeLen == 0) || (prefixSize == INST_MAXIMUM_SIZE))
            return DECRES_INPUTERR;
    ...
    /*
     * Now we decode the instruction and only then we do further prefixes
     handling.
     * This is because the instruction could not be decoded at all, or an
     instruction requires
     * a mandatory prefix, or some of the prefixes were useless, etc...

     * Even if there were a mandatory prefix, we already took into account its
     size as a normal prefix.
     * so prefixSize includes that, and the returned size in pdi is simply the
     size of the real(=without prefixes) instruction.
     */
    if (dt == Decode64Bits) {
        if (ps.decodedPrefixes & INST_PRE_REX) {
            /* REX prefix must precede first byte of instruction. */
            ...
            /*
             * We will disable operand size prefix,
             * if it exists only after decoding the instruction, since it might
             * be a mandatory prefix. This will be done after calling
             * inst_lookup in decode_inst.
             */
        }
        /* In 64 bits, segment overrides of CS, DS, ES and SS are ignored. So
         * don't take'em into account. */
        if (ps.decodedPrefixes & INST_PRE_SEGOVRD_MASK32) {
            ps.decodedPrefixes &= ~INST_PRE_SEGOVRD_MASK32;
            prefixes_ignore(&ps, PFXIDX_SEG);
        }
    ...
    /*
     * The reason we copy these two again is because we have to keep track on
     * the input ourselves. There might be a case when an instruction is
     * invalid, and then it will be counted as one byte only. But that
     * instruction already read a byte or two from the stream and only then
     * returned the error. Thus, we end up unsynchronized on the stream. This
     * way, we are totally safe, because we keep track after the call to
     * decode_inst, using the returned size.
     */
    _CodeInfo ci;
    ci.code = code;
    ci.codeOffset = ci.nextOffset = 0;
    ci.codeLen = codeLen;
    ci.dt = dt;
    ci.features = DF_NONE;

    /* Nobody uses codeOffset in the decoder itself, so spare it. */
    _DecodeResult decodeResult = decode_inst(&ci, &ps, pdi);

    /* Set address to the beginning of the instruction. */
    pdi->addr = startInstAddr & addrMask;
    pdi->disp &= addrMask;

    /* Instruction's size should include prefixes. */
    pdi->size += (uint8_t)prefixSize;

    if (pdi->addr + pdi->size != (_OffsetType)ci.code)
        return DECRES_INPUTERR;
