...
typedef enum OpType {
     ...
     * It supports all decoding modes, but if used as a memory indirection it's
     * a 16 bit ModR/M indirection.
     */
    OT_RFULL_M16,
     ...
     * from memory. JMP FAR [EBX] means EBX point to 16:32 ptr.
     */
    OT_MEM16_FULL,
    /* Read one word (seg) and a word/dword/qword (depends on operand size),
       usually SEG:OFF, JMP 1234:1234 */
    OT_PTR16_FULL,
    /* Read one word (limit) and a dword/qword (limit) (depends on operand
       size), used by SGDT, SIDT, LGDT, LIDT. */
    OT_MEM16_3264,
     ...
     * INC/DEC/PUSH/POP/XCHG, REG is extracted from the value of opcode Use a
     * 8bit register
     */
    OT_IB_RB,
    /* Use a 16/32/64bit register */
    ...
    /* MMX registers: */
    OT_MM,
    /* Extract the MMX register from the RM bits this time (used when the REG
       bits are used for opcode extension) */
    OT_MM_RM,
    ...
    /* SSE registers: */
    OT_XMM,
    /* Extract the SSE register from the RM bits this time (used when the REG
       bits are used for opcode extension) */
    OT_XMM_RM,
 ...
 * result code! This is the BASE structure, there are extentions to this
 * structure below.
 */
...
typedef struct {
    ...
    uint8_t s, d;       /* OpType. */
    uint8_t meta;       /* Hi 5 bits = Instruction set class | Lo 3 bits = flow
                           control flags. */
    uint16_t opcodeId;  /* The opcodeId is really a byte-offset into the
                           mnemonics table. */
} _InstInfo;
...
typedef struct {
    ...
    uint8_t flagsEx;  /* 8 bits are enough, in the future we might make it a
                         bigger integer. */
