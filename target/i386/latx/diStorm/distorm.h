 ...
 * Otherwise comment it out, or you will get a linker error of an unresolved
 * symbol... Turned on by default!
 */

#if 0 /*!(defined(DISTORM_STATIC) || defined(DISTORM_DYNAMIC)) */
	/* Define this macro for outer projects by default. */
#define SUPPORT_64BIT_OFFSET
#endif

/* TINYC has a problem with some 64bits library functions, so ignore 64 bit
 * offsets. */
/*#ifdef __TINYC__
...
/* Get the flow control flags of the instruction, see 'features for decompose'
 * below. */
...
/* Decodes modes of the disassembler, 16 bits or 32 bits or 64 bits for AMD64,
 * x86-64. */
typedef enum {
    Decode16Bits = 0,
    Decode32Bits = 1,
...
typedef struct {
    /* Type of operand:
        ...
        O_SMEM: simple memory dereference with optional displacement (a single
       register memory dereference). O_MEM: complex memory dereference (optional
       fields: s/i/b/disp). O_PC: the relative address of a branch instruction
       (instruction.imm.addr). O_PTR: the absolute target address of a far
       branch instruction (instruction.imm.ptr.seg/off).
    */
    uint8_t type; /* _OperandType */

    /* Index of:
        O_REG: holds global register index
        O_SMEM: holds the 'base' register. E.G: [ECX], [EBX+0x1234] are both in
       operand.index. O_MEM: holds the 'index' register. E.G: [EAX*4] is in
       operand.index.
    */
    uint8_t index;
...
/* The instruction is prefixed with a REP, this can be a REPZ, it depends on the
...
typedef struct {
    /* Used by ops[n].type == O_IMM/O_IMM1&O_IMM2/O_PTR/O_PC. Its size is
     * ops[n].size. */
    _Value imm;
    ...
    _OffsetType addr;
    /* General flags of instruction, holds prefixes and more, if
     * FLAG_NOT_DECODABLE, instruction is invalid. */
    uint16_t flags;
    /* Unused prefixes mask, for each bit that is set that prefix is not used
     * (LSB is byte [addr + 0]). */
    uint16_t unusedPrefixesMask;
    /* Mask of registers that were used in the operands, only used for quick
     * look up, in order to know *some* operand uses that register class. */
    ...
    uint8_t size;
    /* Segment information of memory indirection, default segment, or overriden
     * one, can be -1. Use SEGMENT macros. */
    uint8_t segment;
    /* Used by ops[n].type == O_MEM. Base global register index (might be
     * R_NONE), scale size (2/4/8), ignored for 0 or 1. */
    uint8_t base, scale;
    uint8_t dispSize;
    /* Meta defines the instruction set class, and the flow control flags. Use
     * META macros. */
    uint8_t meta;
} _DInst;

/* Static size of strings. Do not change this value. Keep Python wrapper in
 * sync. */
 ...
 * This structure holds all information the disassembler generates per
 * instruction.
 */
typedef struct {
    _WString mnemonic; /* Mnemonic of decoded instruction, prefixed if required
                          by REP, LOCK etc. */
    _WString operands; /* Operands of the decoded instruction, up to 3 operands,
                          comma-seperated. */
...
/* Register masks for quick look up, each mask indicates one of a register-class
 * that is being used in some operand. */
 ...
 * For now R8 - R15 are not supported and non general purpose registers map into
 * same RM.
 */

/*
 * Instructions Set classes:
 * if you want a better understanding of the available classes, look at disOps
 * project, file: x86sets.py.
 */
...
/* The decoder will return only flow control instructions (and filter the others
 * internally). */
#define DF_RETURN_FC_ONLY 4
/* The decoder will stop and return to the caller when the instruction 'CALL'
 * (near and far) was decoded. */
#define DF_STOP_ON_CALL 8
/* The decoder will stop and return to the caller when the instruction 'RET'
 * (near and far) was decoded. */
#define DF_STOP_ON_RET 0x10
/* The decoder will stop and return to the caller when the instruction
 * system-call/ret was decoded. */
#define DF_STOP_ON_SYS 0x20
/* The decoder will stop and return to the caller when any of the branch 'JMP',
 * (near and far) instructions were decoded. */
#define DF_STOP_ON_UNC_BRANCH 0x40
/* The decoder will stop and return to the caller when any of the conditional
 * branch instruction were decoded. */
#define DF_STOP_ON_CND_BRANCH 0x80
/* The decoder will stop and return to the caller when the instruction 'INT'
 * (INT, INT1, INTO, INT 3) was decoded. */
#define DF_STOP_ON_INT 0x100
/* The decoder will stop and return to the caller when any of the 'CMOVxx'
 ...
 * JCXZ, JO, JNO, JB, JAE, JZ, JNZ, JBE, JA, JS, JNS, JP, JNP, JL, JGE, JLE, JG,
 * LOOP, LOOPZ, LOOPNZ.
 */
...
extern _DecodeResult decode_instruction(uint8_t *ins_addr, _DInst *pdi,
                                        _DecodeType dt);
